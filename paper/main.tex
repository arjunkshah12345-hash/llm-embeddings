...
\graphicspath{{../results/long_50k/figures/}{../results/rank_sweep/figures/}{../results/secondary/figures/}{../results/long_50k/mechanism/}{../results/long_50k/embedding_eval/figures/}}
...
\end{equation}
where $A\in\mathbb{R}^{V\times r}$, $B\in\mathbb{R}^{D\times r}$, and $s=\alpha/r$. The main rank-8 experiment uses $\alpha=8$, so $s=1$. The extra parameter cost is $2r(V+D)$, much smaller than adding a second full vocabulary matrix when $r\ll D$.
...
Weight tying was introduced as a practical and theoretically motivated parameter-sharing method by Press and Wolf and by Inan, Khosravi, and Socher \citep{press2017using,inan2017tying}. Press and Wolf also reported that the tied representation evolves more like the output embedding than the untied input embedding, motivating pathway-specific diagnostics. Work on the geometry and role distinction of input and output representations includes Gulordava et al. and Derby, Miller, and Devereux \citep{gulordava2018represent,derby2020analysing}.
...
The tied model uses one parameter for lookup and projection. The untied model copies the tied initialization into independent input and output parameters. The partial model starts with zero effective corrections and therefore the same initial function as tied. The capacity control is a tied model with a zero-initialized, bias-free residual bottleneck MLP applied to the final hidden states. Its bottleneck width is 1,056, giving 811,008 additional parameters, approximately matching the rank-8 partial model's 810,256 parameters without separating input and output roles. Study 1 additionally includes input-only and output-only corrections.
...
The validator requires a complete seed-by-condition matrix, matching configurations and pinned dataset hashes, finite metrics, a validation record exactly at the declared final step, and exact equality of actual token exposure and sampled-offset digests. All intervals below are descriptive bootstrap intervals over paired seeds; with $n=3$, they describe the observed matched runs rather than providing large-sample inferential evidence.
...
\begin{table}[t]
\centering
\caption{Fresh 50k primary study. Values are means across three seeds; intervals are descriptive bootstrap intervals for final loss.}
\label{tab:primary}
...
\begin{table}[t]
\centering
\caption{Three-seed 10k rank replication. Paired differences are partial rank minus tied; intervals are descriptive bootstrap intervals and lower loss is better.}
\label{tab:rank}
...
\begin{table}[t]
\centering
\caption{Three-seed path-intervention effects at 10k. Each value is partial minus tied final validation loss; intervals are descriptive bootstrap intervals.}
\label{tab:interventions}
\resizebox{\linewidth}{!}{\input{generated/intervention_table.tex}}
\end{table}
...
Recent work by Lopardo et al. reports that tied embeddings become biased toward the output space and attributes part of that effect to output-gradient dominance early in training \citep{lopardo2026weight}. Our final-step effective gradient measurements are closer to parity in this smaller-model regime: the 50k output-to-input ratios are near one for tied and rank-8 partial models. These observations are not directly contradictory because the studies differ in model scale, gradient measurement, and training phase. They instead suggest that measurable role asymmetry need not imply that decoupling improves language-modeling loss. Early-training trajectories, larger models, and other diagnostics could still reveal dynamics that are averaged away by our final-step measurements.
...
The final WikiText-2 study uses GPT-2 BPE tokenization, a six-layer decoder with six attention heads and width 384, context length 256, batch size 2, AdamW, learning rate $3\times10^{-4}$ with a 100-step warmup and cosine decay to $3\times10^{-5}$, weight decay 0.1, and gradient clipping at 1.0. Each seed receives 25,600,000 training tokens over 50,000 optimizer steps. Validation uses the same 20 deterministic batches every 200 steps and records the required final step 49,999. The rank replication uses the same configuration for 10,000 steps and ranks 1, 2, 4, 8, 16, and 32. The rank-8 main experiment uses adapter $\alpha=8$, so the correction scale $s=\alpha/r$ equals 1.
...
All primary comparisons are paired by seed. The reported intervals are descriptive percentile-bootstrap intervals over the three paired seed differences; they describe the observed matched runs and are not a substitute for a larger-sample inferential analysis. A recovered fraction is omitted when the tied-versus-untied denominator is not meaningfully positive. The fairness validator rejects missing cells, config or dataset mismatches, unequal sampled-token digests, non-finite metrics, and final validation records at the wrong step.
